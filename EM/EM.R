library(ggplot2)
library(stats)
setwd("/Users/jason/Documents/projects/ODU/PHYS413/EM/")
source("functions.R")
source("extrema.R")
data <- read.table("em-3.csv", header = TRUE, sep = ",")
attach(data)
fit  <- lm(y~x)
m <- summary(fit)
se <- m$sigma
...
bars <- geom_errorbar(limits, width=.00005)
...
a <- ggplotRegression(fit, "", expression(V), expression(B^2), bars)
print(a)
